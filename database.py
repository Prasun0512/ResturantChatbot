#Setting threshold with min score for dense search
#in case match just fire simple query without min_score
...
class ElasticTransformers(object):
    ...
    def create_index(self, index_name=None, index_file=None):
        ...
        if index_file or self.index_file:
            ...
            # }
        import json
        index_spec1=json.loads(index_spec)
        #print("index_spec[settings]: ",index_spec1["settings"])
        #print("index_spec[mappings]: ",index_spec)
        self.es.indices.create(index=index_name, settings=index_spec1["settings"], mappings=index_spec1["mappings"])
            ...
            script_query = {
                "script_score": {
                    "query": {"match": {
                        field: {
                            "query": query,
                            "minimum_should_match":"75%"   # set as per number of % keyword match
                            }
                        }
                    },
                    "script": {
